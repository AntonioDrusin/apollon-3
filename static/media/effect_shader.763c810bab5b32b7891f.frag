...
void main()
{
    ...
    if (aspectRatio >= 1.0) {
        uv.x = vUv.x;
        uv.y = vUv.y / aspectRatio;
    } else {
        uv.x = vUv.x * aspectRatio;
        uv.y = vUv.y;
    }
    uv.x = (vUv.x - 0.5) / iZoom + 0.5;
    uv.y = (vUv.y - 0.5) / iZoom + 0.5;

    // vec2 uv = (vUv - 0.5 * iResolution.xy) / iResolution.y;
    vec3 c = sin(iTime * 2.0 * vec3(.234, .324,.768)) * 0.4 + 0.6;
    vec3 col = vec3(0);

    float fft = iAmplitude; // texelFetch(iChannel0, ivec2(76.0, 0.), 0).x / 2.0 + 0.5;
